...
class LocalScan{
    ...
    @discardableResult func findSystemList(_ mac: String?) -> [CBPeripheral]?{
        systemList = BluetoothManager.shared.manager.retrieveConnectedPeripherals(withServices: BluetoothManager.shared.servicesUUID)
        let uuidStr = getUuid(mac: mac)
        systemList?.forEach({
            BluetoothManager.shared.writeLog("retrieveConnectedPeripherals" + String(describing: $0.name) + $0.identifier.uuidString + "连接状态 === \($0.state)" + "macAddress == \(String(describing: mac))",method: "findSystemList",className:"LocalScan")
        })
        if isContainDevice(uuidStr) {
            BluetoothManager.shared.writeLog("Device is found in the system Bluetooth list == \(String(describing: mac))",method: "findSystemList",className:"LocalScan")
            return systemList
        }
        if uuidStr.isEmpty {
            return systemList
        ...
        if isContainDevice(uuidStr) == false {
            if let uuid = UUID(uuidString: uuidStr) {
                let list = BluetoothManager.shared.manager.retrievePeripherals(withIdentifiers: [uuid])
                list.forEach({
                    BluetoothManager.shared.writeLog("retrievePeripherals" + String(describing: $0.name) + $0.identifier.uuidString + "连接状态 === \($0.state)" + "macAddress == \(String(describing: mac))",method: "findSystemList",className:"LocalScan")
                })
                if list.count > 0 {
